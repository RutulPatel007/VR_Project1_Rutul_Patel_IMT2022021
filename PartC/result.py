...
def load_dataset(csv_path):
    ...
    return df[df['with_mask'] == True]  # Filter only "with_mask" images
...
def process_images(csv_path, image_folder, output_folder, method='threshold'):
    ...
    print(f"Mean IoU ({method}): {np.mean(iou_scores):.4f}")
    print(f"Mean Dice Score ({method}): {np.mean(dice_scores):.4f}")

csv_path = "../MSFD/1/dataset.csv"
image_folder = "../MSFD/1/img"
output_folder = "../MSFD/1/segmented_output"
